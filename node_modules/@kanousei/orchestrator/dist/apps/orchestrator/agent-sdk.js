...
import { urlSchema, crawlResultSchema, assetResultSchema, coverageSchema, designSystemSchema, synthResultSchema, verificationReportSchema } from '@kanousei/validation';
// ============================================================================
...
function validateEnv() {
    ...
    const authMethods = ['CLAUDE_CODE_OAUTH_TOKEN', 'ANTHROPIC_API_KEY'];
    const missing = required.filter(key => !process.env[key]);
    const hasAuth = authMethods.some(key => process.env[key]);
    if (missing.length > 0) {
        throw new Error(`Missing required environment variables: ${missing.join(', ')}`);
    }
    if (!hasAuth) {
        throw new Error(`Missing authentication: Set either CLAUDE_CODE_OAUTH_TOKEN or ANTHROPIC_API_KEY`);
    }
    if (process.env.CLAUDE_CODE_OAUTH_TOKEN) {
        console.log('✅ Using Claude Code OAuth token (from subscription)');
    }
    else {
 ...
 */
const scoutTool = tool('scout', `Crawl a website to discover all pages, assets, and structure.
 ...
 */
const downloadAssetsTool = tool('download_assets', `Download all assets discovered during scouting.
 ...
 * Check coverage tool
 */
const checkCoverageTool = tool('check_coverage', `Check asset download coverage percentage.
 ...
 */
const extractDesignSystemTool = tool('extract_design_system', `Analyze the website and extract design system tokens.
 ...
 */
const synthesizePagesTool = tool('synthesize_pages', `Generate Next.js pages and components from extracted design system.
 ...
 * Verify build tool
 */
const verifyBuildTool = tool('verify_build', `Verify the generated Next.js site matches the original.
    ...
    try {
        ...
        if (!result.passed) {
            ...
            if (!result.visualDiff.passed)
                issues.push('visual diff failed');
            if (!result.linkReport.ok)
                issues.push(`${result.linkReport.broken.length} broken links`);
            if (!result.a11y.ok)
                issues.push(`${result.a11y.issues.length} A11y issues`);
            return {
                content: [{
                        type: 'text',
