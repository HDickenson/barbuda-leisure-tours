...
const Montage: React.FC<MontageProps> = ({ images, className = '', layout = 'auto' }) => {
  ...
  return (
    <div className={`montage-root ${isMounted ? 'montage-root--mounted' : ''} ${className}`}>
      <div className={`montage-grid montage-grid--${effectiveLayout}`}>
        {images.map((image, index) => {
          ...
          return (
            ...
            >
              <img src={image.src} alt={image.alt ?? ''} loading="lazy" className="montage-img" />
            </div>
          )
        })}
      </div>
