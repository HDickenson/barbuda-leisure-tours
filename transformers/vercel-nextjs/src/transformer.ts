...
// Types for Elementor extracted data
 ...
 */
export function transformPageToNextJS(page: ElementorPage): {
  component: string;
  contentData: any;
  styles: string;
} {
 ...
 */
function extractContentData(page: ElementorPage): any {
  return {
    title: page.title,
    slug: page.slug,
 ...
 */
function generateComponentCode(componentName: string, page: ElementorPage): string {
  const imports = generateImports(page);
  const sectionsCode = page.sections.map((section, idx) =>
    generateSectionCode(section, idx)
  ).join('\n\n');
 ...
 */
function generateImports(page: ElementorPage): string {
  const imports = new Set<string>(['Image']);

  // Detect which components are needed
  page.sections.forEach(section => {
    section.columns.forEach(column => {
      column.widgets.forEach(widget => {
        if (widget.widgetType === 'image') imports.add('Image');
        if (widget.widgetType === 'button') imports.add('Button');
        if (widget.widgetType === 'heading') imports.add('Heading');
      });
    });
  });

  return `import Image from 'next/image';
${Array.from(imports).filter(i => i !== 'Image').map(i =>
  `import { ${i} } from '@/components/ui/${i.toLowerCase()}';`
).join('\n')}`;
}
 ...
 */
function generateSectionCode(section: ElementorSection, idx: number): string {
  const sectionClasses = generateSectionClasses(section);
  const backgroundStyle = generateBackgroundStyle(section.background);

  const columnsCode = section.columns.map((column, colIdx) =>
    generateColumnCode(column, colIdx)
  ).join('\n        ');
 ...
 */
function generateColumnCode(column: ElementorColumn, idx: number): string {
  const columnClasses = generateColumnClasses(column);

  const widgetsCode = column.widgets.map((widget, widgetIdx) =>
    generateWidgetCode(widget, widgetIdx)
  ).join('\n            ');
 ...
 */
function generateWidgetCode(widget: ElementorWidget, idx: number): string {
 ...
 */
function generateHeadingWidget(widget: ElementorWidget): string {
 ...
 */
function generateTextEditorWidget(widget: ElementorWidget): string {
 ...
 */
function generateImageWidget(widget: ElementorWidget): string {
 ...
 */
function generateButtonWidget(widget: ElementorWidget): string {
 ...
 */
function generateCardWidget(widget: ElementorWidget): string {
 ...
 */
function generateSectionClasses(section: ElementorSection): string {
  const classes = ['py-12'];

  if (section.padding) {
    const p = section.padding;
    if (p.top) classes.push(`pt-[${p.top}${p.unit}]`);
    if (p.bottom) classes.push(`pb-[${p.bottom}${p.unit}]`);
  }
 ...
 */
function generateColumnClasses(column: ElementorColumn): string {
  const colSpan = Math.round((column.width / 100) * 12);
  const classes = [`col-span-${colSpan}`];

  if (column.padding) {
    const p = column.padding;
    if (p.top && p.top !== '0') classes.push(`pt-[${p.top}${p.unit}]`);
    if (p.bottom && p.bottom !== '0') classes.push(`pb-[${p.bottom}${p.unit}]`);
    if (p.left && p.left !== '0') classes.push(`pl-[${p.left}${p.unit}]`);
    if (p.right && p.right !== '0') classes.push(`pr-[${p.right}${p.unit}]`);
  }
 ...
 */
function generateBackgroundStyle(background?: any): string | null {
 ...
 */
function generateTailwindStyles(page: ElementorPage): string {
 ...
 */
function escapeBackticks(text: any): string {
  if (typeof text !== 'string') return '';
 ...
 */
export function transformAllPages(inputDir: string, outputDir: string): void {
  const fs = require('fs');
  ...
  const files = fs.readdirSync(inputDir)
    .filter((f: string) => f.startsWith('page-') && f.endsWith('.json'));

  console.log(`\nTransforming ${files.length} pages...\n`);

  files.forEach((file: string) => {
    const pagePath = join(inputDir, file);
    const page: ElementorPage = JSON.parse(fs.readFileSync(pagePath, 'utf-8'));
    ...
    const componentPath = join(pagesDir, `${page.slug}.tsx`);
    writeFileSync(componentPath, component, 'utf-8');

    // Write content data
    const dataPath = join(dataDir, `${page.slug}.json`);
    writeFileSync(dataPath, JSON.stringify(contentData, null, 2), 'utf-8');
  });
